...
interface ExcluirInstanciaModalProps {
  instancia: Instancia;
  onFechar: () => void;
  onConfirmar: (id: string) => Promise<void>;
}

export function ExcluirInstanciaModal({ instancia, onFechar, onConfirmar }: ExcluirInstanciaModalProps) {
  ...
  async function handleConfirmar() {
    ...
    setErro(null);
    try {
      await onConfirmar(instancia.id);
    } catch (e) {
      setErro(e instanceof Error ? e.message : "Não foi possível excluir a instância.");
      setExcluindo(false);
    }
  }
