// Ablaw IA — testa de verdade a conexão com a API da Anthropic usando o
...
Deno.serve(async (req) => {
  ...
  if (!ANTHROPIC_API_KEY) {
    return jsonResponse({ erro: "ANTHROPIC_API_KEY não configurada" }, 500);
  }
  ...
  const texto = corpoResposta?.content?.[0]?.text ?? "";
